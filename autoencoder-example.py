...
Link: https://youtu.be/6S2v7G-OupA
"""
...
df = dataframe[['datetime', 'back_angle']]
df['Date'] = pd.to_datetime(df['datetime'])

print(df.head())

# sns.lineplot(x=df['Date'], y=df['back_angle'])
...
train, test = df.loc[df['Date'] <= '2011-12-01 11:30:00'], df.loc[df['Date'] > '2011-12-01 11:35:00']
...
trainX, trainY = to_sequences(train[['back_angle']], train['back_angle'], seq_size)
testX, testY = to_sequences(test[['back_angle']], test['back_angle'], seq_size)
...
max_trainMAE = [0.8 * max(trainMAE)] * len(test[seq_size:])  #or Define 90% value of max as threshold.
...
#Plot anomalies
sns.lineplot(x=anomaly_df['Date'], y=scaler.inverse_transform(anomaly_df['back_angle']))
sns.scatterplot(x=anomalies['Date'], y=scaler.inverse_transform(anomalies['back_angle']), color='r')
